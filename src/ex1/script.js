...
function addEnter(event) {
    if ((event.code === 'Enter')&&(document.querySelector('#newToDo input').value.length!=0)) add()
  }
...
function add () {
    if(document.querySelector('#newToDo input').value.length == 0) {
        ...
        remove.addEventListener('click', function(){this.parentNode.style.opacity = '0'; count.textContent--;clickButtonClearAll(); setTimeout(() => this.parentNode.remove()(), 200); });
