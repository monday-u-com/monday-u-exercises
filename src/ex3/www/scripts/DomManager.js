...
import { POKEMON_IMAGE_DIV, TASK_HTML } from './GlobalConses';
...
export default class DomManager {
  ...
  AddNewTask(id, task_object, delete_call_back, complete_call_back) {
    ...
    } else if (task_object.id === 'Error') { task_text.innerHTML = task_object.data; } else { task_text.innerHTML = task_object.name; }// adds text to task
  ...
  PokemonImageElement(task_object) {
    ...
    Object.entries(task_object.images).forEach((image_attribute, index) => {
      ...
      if (index === 0) { pokemon_image.classList.add('show'); } else { pokemon_image.classList.add('hide'); }
  ...
  RenderDomFromArray(todos, delete_call_back, complete_call_back) {
    ...
    } else { this.task_container.classList.add('empty'); }
    this.UpdateTaskCounter();
  }
  ...
  PokemonImageAnimation() {
    ...
    image_showing.forEach((image) => {
      ...
      if (next_image === null) { next_image = image.parentNode.firstChild; }
